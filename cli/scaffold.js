...
// so it runs through safeJSONForHTMLScript (from sdk/olladroid.js) to neuter
...
function buildAppConfig(opts) {
  ...
  cfg.host = opts.host;
  cfg.sdkVersion = Olladroid.VERSION;
...
function buildManifest(opts) {
  return {
    ...
    description: opts.description || (opts.appName + ' — a local AI app scaffolded by olladroid'),
...
function buildIconSVG(opts) {
  ...
  const hue = hashHue(opts.slug || opts.appName || 'olladroid');
...
function buildServiceWorker(opts) {
  // Network-first service worker: always try the network, fall back to
  // the cache only when offline. This costs us the purist "cache-first
  // PWA" trick but avoids the trap we hit on PR 4 device testing, where
  // re-scaffolding an app produced a new index.html that Chrome refused
  // to load because the stale cache-first SW kept serving the old HTML.
  // The `fetch` handler also fills the cache on every successful network
  // fetch so offline mode still works after one online visit.
  //
  // Scoped per-slug cache name means a CLI regeneration does not collide
  // with other scaffolded apps on the same origin. Stale entries for the
  // same slug are cleared on `activate`.
  const cacheName = 'olladroid-' + opts.slug + '-v2';
  return (
...
function composeIndexHTML(sources, opts) {
  ...
  const configJSON = Olladroid.safeJSONForHTMLScript(buildAppConfig(opts));
  const replacements = {
...
async function readTemplateSources(repoRoot, templateName) {
  ...
  const sdkPath = path.join(repoRoot, 'sdk', 'olladroid.js');
